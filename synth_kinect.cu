/*
 * synth_kinect.cu — Realistic Kinect v2 ToF IR frame generator
 * ─────────────────────────────────────────────────────────────
 * Simulates the noise profile of a Microsoft Kinect v2 (ToF) sensor
 * without needing physical hardware.  Useful for testing the CUDA
 * vision pipeline and blob detection offline.
 *
 * Noise model (empirically derived from Kinect v2 characterisation
...
// ── Kinect v2 IR optics ───────────────────────────────────────────────────────
// Focal length of the IR projector in pixels (from Kinect v2 calibration spec)
#define KINECT_FOCAL_PX   364.0f
    ...
    unsigned int frame_seed)
{
    ...
    // This is the dominant noise source in Kinect v2 at long distances.
    ...
    for (int k = 0; k < n_objects; ++k) {
        ...
        float sigma_px  = (obj.radius_m / d_clamped) * KINECT_FOCAL_PX;
